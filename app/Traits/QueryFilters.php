...
class QueryFilters
{
    ...
    public function __construct()
    {
        ...
        $this->sessionFilterSearch = isset($this->sessionFilterData['search']) ?
            $this->sessionFilterData['search'] : null;
    }
    ...
    protected function filters(): array
    {
        ...
        if (empty($filters) || (count($this->request->all()) == 1 && $this->request->has('page'))) {
    ...
    private function search()
    {
        ...
        if (empty($search) || (count($this->request->all()) == 1 && $this->request->has('page'))) {
    ...
    /**
     * Modificado a função, removido o código
     *
        ...
        $this->builder->where(function ($query) use ($keyword, $columns, $columnsExceptCollate) {
            if (count($columns) > 0) {
                foreach ($columns as $key => $column) {
                    ...
                    $collate = SqlHelper::getCollateForInsensitiveComparison();
                    $keyword = SqlHelper::quoteEscaped($keyword);

                    $query->$clause("$column $collate  LIKE ?", [
                        "%$keyword%"
                    ]);
                }
            }
